...
// Try common interpreter locations (Unix venv, Windows venv, or global `python`)
$candidates = [
...
foreach ($candidates as $candidate) {
    if ($candidate === 'python') {
        $check = @shell_exec(escapeshellcmd('python') . " --version 2>&1");
...
// Build command and capture stderr for easier debugging
$command = escapeshellcmd($pythonPath) . " " .
           escapeshellarg($scriptPath) . " " .
           escapeshellarg($jsonInput) . " 2>&1";
